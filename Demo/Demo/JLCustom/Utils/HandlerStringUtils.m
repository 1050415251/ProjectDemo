...
#import "HandlerStringUtils.h"

@implementation HandlerStringUtils
...
#pragma mark ------------------ 每隔n位添加字符
///每隔4个字符添加一个，的字符串算法(倒着添加如 1000,0000,00)
/// - parameters
///   - number:要出理的字符串
///   - chart:要插入的字符串
///   - index:每隔n为插入
+ (NSString *)stringInsertChart:(NSString *)number chart:(NSString *)chart index:(int)index
{
    ...
    for (int i = 0; i < number.length; i++) {
        ...
        if (count == index) {
            doneTitle = [NSString stringWithFormat:@"%@%@", doneTitle,chart];
            count = 0;
        }
    }
   // NSLog(@"%@", doneTitle);
...
#pragma mark ------------------ 每隔n位添加字符(倒着)
///每隔4个字符添加一个，的字符串算法(倒着添加如 10,0000,0000)
/// - parameters
///   - number:要出理的字符串
///   - chart:要插入的字符串
///   - index:每隔n为插入
+ (NSString *)stringbackInsertChart:(NSString *)number chart:(NSString *)chart index:(int)index
{
    ...
    for (int i = 0; i < number.length; i++) {
        ...
        doneTitle = [doneTitle stringByAppendingString:[number substringWithRange:NSMakeRange(i, 1)]];
        if (count%index == 0 && i != number.length - 1) {
