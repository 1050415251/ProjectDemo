...
public class PhotoToolBar: UIView {
    ...
    fileprivate var cancleItem:UIButton = {
        $0.setTitle("取消", for: UIControl.State())
        $0.setTitleColor(UIColor.white, for: UIControl.State())
        $0.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        $0.translatesAutoresizingMaskIntoConstraints = false
        $0.setContentCompressionResistancePriority(UILayoutPriority(rawValue: 999), for: .horizontal)
        $0.setContentCompressionResistancePriority(UILayoutPriority(rawValue: 999), for: .vertical)
        return $0
    }(UIButton(type: .custom))
    
    fileprivate var confirmItem:UIButton = {
        $0.setTitle("确定", for: UIControl.State())
        $0.setTitleColor(UIColor.white, for: UIControl.State())
        $0.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        $0.translatesAutoresizingMaskIntoConstraints = false
        $0.setContentCompressionResistancePriority(UILayoutPriority(rawValue: 999), for: .horizontal)
