...
extension UIImage:ImageEditor{
    ...
    var properties:Dictionary<String,AnyObject> {
        get{
            ...
            let lock = SPinLock()
            lock.lock()
            guard let data  =  self.jpegData(compressionQuality: 0.9)  else{ return propertise}
    ...
    func scaleAndCrop(size: CGSize) -> UIImage {
        ...
        UIGraphicsEndImageContext()
        //从新图片中剪切
        //DEBUGMODE.DEBUGPRINT("newImage size \(newImage.size)")
        ...
        //DEBUGMODE.DEBUGPRINT("smaillImage size \(smaillImage.size)")
